  ...
  return (
    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-[0_18px_40px_rgba(15,17,21,0.08)]">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h2 className="text-xl font-semibold text-ink-900">Activities 🏃‍♀️</h2>
          ...
          <table className="min-w-full border-separate border-spacing-y-3 text-sm">
            ...
            <tbody>
              {activities.map((activity) => (
                <tr key={activity.id || activity._id} className="bg-cloud-100 shadow-[0_12px_20px_rgba(15,17,21,0.06)]">
                  <td className="rounded-l-2xl px-4 py-3 font-medium text-ink-900">
                    {activity.name || activity.title || 'Activity'}
                  </td>
                  <td className="px-4 py-3 text-ink-700">{activity.type || activity.category || '-'}</td>
                  <td className="px-4 py-3 text-ink-700">{activity.duration || activity.minutes || '-'}</td>
